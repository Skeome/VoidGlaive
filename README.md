...
- **`stoat-bot.py`** — Stoat, built with [stoat.py](https://stoat.chat) (prefix commands)
...
Before running, log into [stt.gg](https://stt.gg) as your bot account and **fully complete its profile** — set an avatar, bio, and banner. Stoat's onboarding check runs at the WebSocket authentication layer, and a bot with an incomplete profile will be rejected with a misleading `InvalidSession` error even if the token is valid.
...
| `STOAT_BOT_TOKEN` | ✅ | Stoat | Bot token from the Stoat Developer Portal |
...
To get a user or guild ID, enable **Developer Mode** in your client settings, then right-click any user or server and select **Copy ID**.
